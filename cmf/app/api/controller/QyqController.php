...
class QyqController extends RestBaseController
{
    ...
    */
    public function add_qyq() {
            ...
            $parm['time'] = time();
            unset($parm['gameIds']);
            $apiurl = get_api_url()."createGroup.action";
    ...
    public function getid_by_detail() {
        $config = [
            'key' =>"Qhbg8ILxlXG2NU6a", //加密key
            'iv'  =>"Qhbg8ILxlXG2NU6a", //保证偏移量为16位
    ...
    public function stop_qyq() {
        if(input('post.')){
            ...
            $parm = input('post.');
            $parm['time'] = time();
            $apiurl = $apiurl = get_api_url()."forbidGroup.action";
            $parm['sign'] = checkSign($parm);
    ...
    public function stop_dlm() {
        if(input('post.')){
            ...
            $parm['time'] = time();
            $parm['optType'] = 2;
            $apiurl = $apiurl = get_api_url()."updateGroup.action";
            $parm['sign'] = checkSign($parm);
            $info = cmf_api_request($apiurl,$parm);
            $res_info = decrypt_info($info);
            if($res_info['code']===0){
    ...
    public function zy_cy() {
        if(input('post.')){
            ...
            $parm['time'] = time();
            $parm['retainCredit'] = 0;
            $apiurl = $apiurl = get_api_url()."moveGroupUser.action";
            $parm['sign'] = checkSign($parm);
            $info = cmf_api_request($apiurl,$parm);
            $res_info = decrypt_info($info);
            if($res_info['code']===0){
    ...
    public function fireUser() {
        if(input('post.')){
            ...
            $parm = input('post.');
            $parm['time'] = time();
            $apiurl = $apiurl = get_api_url()."fireUser.action";
            $parm['sign'] = checkSign($parm);
            $info = cmf_api_request($apiurl,$parm);
            $res_info = decrypt_info($info);
            if($res_info['code']===0){
    ...
    public function move_qz() {
        if(input('post.')){
            ...
            $parm = input('post.');
            $parm['time'] = time();
            $apiurl = $apiurl = get_api_url()."changeMaster.action";
            $parm['sign'] = checkSign($parm);
            $info = cmf_api_request($apiurl,$parm);
            if(empty($info)){
    ...
    public function add_member() {
        if(input('post.')){
            ...
            $parm['time'] = time();
            $parm['optType'] = 1;
            $apiurl = $apiurl = get_api_url()."selfRegister.action";
            $parm['sign'] = checkSign($parm);
            $info = cmf_api_request($apiurl,$parm);
            $res_info = decrypt_info($info);
            if($res_info['code']===0){
